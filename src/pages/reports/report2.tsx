...
function Report1() {
  ...
  const getList = () => {
    ...
    services.g
      ...
      .catch((error) => {
        console.error('Error loading report 2:', error);
        Message.error('Failed to load workshops by region report. Please try again.');
        setColumns([]);
        setData([]);
      })
      .finally(() => {
  ...
  const handleExport = async () => {
    try {
      await exportExcel(columns, data, 'WORKSHOPS BY REGION + YEAR');
