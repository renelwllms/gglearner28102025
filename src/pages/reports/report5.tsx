...
function Report1() {
  ...
  const getList = () => {
    ...
    services.g
      ...
      .catch((error) => {
        console.error('Error loading report 5:', error);
        Message.error('Failed to load tutor learner numbers report. Please try again.');
        setColumns([]);
        setData([]);
      })
      .finally(() => {
  ...
  const handleExport = async () => {
    try {
      await exportExcel(columns, data, 'TUTOR + LEARNER NUMBERS + YEAR');
