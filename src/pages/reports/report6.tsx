...
function Report1() {
  ...
  const getList = () => {
    ...
    services.g
      ...
      .catch((error) => {
        console.error('Error loading report 6:', error);
        Message.error('Failed to load ethnicity learner numbers report. Please try again.');
        setColumns([]);
        setData([]);
      })
      .finally(() => {
  ...
  const handleExport = async () => {
    try {
      await exportExcel(columns, data, 'ETHNICITY + LEARNER NUMBERS + YEAR');
